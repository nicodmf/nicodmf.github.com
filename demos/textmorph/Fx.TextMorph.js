...
Fx.TextMorph = new Class({
	...
	*/	
	...
	simplyShy : function(element) {
		...
		var patterns = "/\([aeiéèïôâêàouyAEIOUY][bcdfghjklmnpqrstvwxzBCDFGHJKLMNPQRSTVWXZ]\)\([bcdfghjklmnpqrstvwxzBCDFGHJKLMNPQRSTVWXZ]\)/";
		element.set('text', this.pregReplace(patterns, replacements, element
				.get('text')));
		patterns = "/\([aeiéèïôâêàouyAEIOUY]\)\([bcdfghjklmnpqrstvwxzBCDFGHJKLMNPQRSTVWXZ][aeiéèïôâêàouyAEIOUY]\)/";
		element.set('text', this.pregReplace(patterns, replacements, element
				.get('text')));
		patterns = "/\([aeiéèïôâêàouyAEIOUY]\)\([aeiéèïôâêàouyAEIOUY]\)/";
		element.set('text', this.pregReplace(patterns, replacements, element
				.get('text')));
	}
})
